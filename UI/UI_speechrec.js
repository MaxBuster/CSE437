if (!('webkitSpeechRecognition' in window)) {
	...
	recognition.onresult = function() {
		...
		for (var i = event.resultIndex; i < event.results.length; ++i) {
	      if (event.results[i].isFinal) {
	        final_transcript += "<p class='my_results col-md-5 pull-right well'>" + myName + ": " + event.results[i][0].transcript + "</p>";
	      } else {
	...
	myUA.on('message', function (msg) {
		...
		msgbody = msgbody.replace("my_results col-md-5 pull-right well", "other_results col-md-5 pull-left well");
